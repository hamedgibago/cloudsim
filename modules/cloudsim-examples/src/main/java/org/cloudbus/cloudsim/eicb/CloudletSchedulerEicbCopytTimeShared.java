...
public class CloudletSchedulerEicbCopytTimeShared extends CloudletScheduler {
	 ...
	 * @post $none
	 */
	public CloudletSchedulerEicbCopytTimeShared() {
		super();
		currentCPUs = 0;
	}

	private static List<activity> BatchProcess(List<activity> AIQ) {
		List<activity> PI = new ArrayList<activity>(); // pending instances
		List<activity> WQ = new ArrayList<activity>(); 
		AIQ.forEach(a -> {
		...
		for (activity a : WQ) {
			if (a.getWaitingTime() >= a.deadline) {
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {	
		...
		//BatchProcess(getCloudletWaitingList());
		
	 	List<ResCloudlet> lsRes= getCloudletWaitingList();
