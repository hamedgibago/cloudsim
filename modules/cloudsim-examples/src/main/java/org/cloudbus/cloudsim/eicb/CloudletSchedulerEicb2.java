...
public class CloudletSchedulerEicb2 extends CloudletSchedulerSpaceShared {
	...
	public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		...
		if (
				//comented by hamed
				//(currentCpus - usedPes) >= cloudlet.getNumberOfPes() &&
				!((activity)cloudlet).acivityType				
				) {
			ResCloudlet rcl = new ResCloudlet(cloudlet);			

			rcl.setCloudletArrivalTime(((activity)cloudlet).arrivingTime);
